...
import { snacksConsumer } from '@/data/index.js'  
</script>

<template>
    <!-- Hero -->
    <ComponentSection spacing="xs" :fullWidth="true" style="padding-bottom: 0px; padding-top: 0px">
        <ComponentHero :title="snacksConsumer.hero.title" :subtitle="snacksConsumer.hero.subtitle"
            :fullWidth="snacksConsumer.hero.fullWidth" :backgroundImage="snacksConsumer.hero.backgroundImage"
            :overlayOpacity="snacksConsumer.hero.overlayOpacity" />
    </ComponentSection>
            ...
            <ComponentGrid :columns="{ base: 1, md: 2 }" gap="xl" align="center">
                <div>
                    <h2
                        ...
                        {{ snacksConsumer.productOverview.title }}
                    </h2>
                    <p class="font-body text-lg text-pickled-bluewood-700 leading-relaxed mb-6">
                        {{ snacksConsumer.productOverview.description }}
                    </p>
                </div>
                <div class="bg-[hsl(var(--cello-100))] p-8 rounded-lg">
                    ...
                    <ul class="space-y-3">
                        <li v-for="feature in snacksConsumer.productOverview.features" :key="feature.title"
        ...
        <ComponentContainer size="2xl" padding="responsive" :constrainWidth="true">
            <div class="text-center mb-12">
                <h2
                    ...
                    {{ snacksConsumer.productCategories.title }}
                </h2>
                <p class="font-body text-lg text-pickled-bluewood-700 leading-relaxed max-w-4xl mx-auto">
                    {{ snacksConsumer.productCategories.description }}
                </p>
            </div>
            <ComponentGrid :columns="{ base: 1, md: 2, lg: 4 }" gap="lg">
                <ComponentProductCard v-for="category in snacksConsumer.productCategories.categories"
                    :key="category.title" :product="{
                        title: category.title,
                        image: category.image,
                        specs: category.specs
                    }" variant="detailed" imageFit="object-cover" />
            </ComponentGrid>
        </ComponentContainer>
    </ComponentSection>

    <!-- CTA Section -->
    <ComponentCtaGetInTouch title="Partner With Us to Bring Flavor to Every Market"
        description="Work with E-Sunrise Australia to develop sauces and condiments that balance authentic taste with industrial scalability. From retail shelves to foodservice kitchens, we deliver certified quality, packaging flexibility, and consistent supply your business can trust."
        buttonText="Start Your Flavor Journey" backgroundColor="pickled-bluewood-100" />
</template>
