export const footerLinks = [
    {
        title: 'Quick Links',
        links: [
            { text: 'Our Company', path: '/our-company' },
            { text: 'Our Businesses', path: '/our-businesses' },
            { text: 'Industries', path: '/our-businesses/industries' },
            { text: 'Products', path: '/products' },
            { text: 'News', path: '/news' },
            { text: 'Contact', path: '/contact' }
        ]
    },

    {
        title: 'Our Company',
        links: [
            { text: 'Mission & Vision', path: '/our-company/mission-and-vision' },
            { text: 'Our History', path: '/our-company/history' },
            { text: 'News', path: '/news' },
        ]
    },
    {
        title: 'Our Businesses',
        links: [
            { text: 'Import & Distribution', path: '/our-businesses/import-and-distribution' },
            { text: 'OEM / Private Label', path: '/our-businesses/oem-and-private-label' },
            { text: 'Industries', path: '/our-businesses/industries' },
        ]
    },
    {
        title: 'Products',
        links: [
            { text: 'Fresh Produce', path: '/products/fresh-produce' },
            { text: 'Processed Garlic', path: '/products/processed-garlic' },
            { text: 'Condiments & Sauces', path: '/products/condiments-sauces' },
            { text: 'Snacks & Consumer Products', path: '/products/snacks-consumer' },
            { text: 'Broader Agriculture', path: '/products/broader-agriculture' },
        ]
    },
]

export const socialLinks = [
    { name: 'Facebook', icon: 'facebook', url: 'https://www.facebook.com/ESunriseAU' },
    { name: 'Rednote', icon: 'rednote', url: 'https://www.xiaohongshu.com/user/profile/668f3c91000000000f035ee5' },
    { name: 'LinkedIn', icon: 'linkedin', url: 'https://www.linkedin.com/company/e-sunrise/' },
    { name: 'Instagram', icon: 'instagram', url: 'https://www.instagram.com/esunrise_australia/' }
] 